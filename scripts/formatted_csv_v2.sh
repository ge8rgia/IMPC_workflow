...
mkdir -p "$OUTPUT_DIR" #Creates fornat.csvs directory if not present


printf "Retrieving headers (in parralel)...\n"

CPU_CORES=8  #Selecting amount of CPU Cores to run this task in parralel
...
for csv_file in "$INPUT_DIR"/*.csv; do
    ...
    # 2. BEGIN block: awk splits this string into its own 'header_array'
    # 3. Main block: awk reads the file line-by-line, storing values in a 'data' array (e.g., data["param_name"] = "value")
    # 4. END block: After reading the whole file, awk loops through the 'header_array' (in the correct order)
    #    and prints the corresponding value from its 'data' array.
