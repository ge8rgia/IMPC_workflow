...
# description format --> should have underscore between words only

# Missing _
sum(grepl(" ", IMPC_parameter$description)) #92 do not have underscores separating words and are separated by spaces
paramdescrip_missing_underscore <- IMPC_parameter$description[grepl(" ", IMPC_parameter$description)]
paramdescrip_missing_underscore
...
# Standard format should begin with lowercase
all(grepl("^[a-z]", IMPC_parameter$description)) #FALSE
# to view them
paramdescrip_not_lowercase <- IMPC_parameter$description[!grepl("^[a-z]", IMPC_parameter$description)]
paramdescrip_not_lowercase # some in non-standard format, also shows missing descriptions (NAs), and some beginning with numbers 
...
IMPC_parameter_clean <- IMPC_parameter %>%
  ...
  mutate(
    description = description %>%
      str_replace_all(" ", "_") %>% #Replacing spaces with _
      str_replace("^_+", "") %>% #Removing leading _
      str_replace("_+$", "") %>% #Removing ending _
      str_replace("^([A-Z])", tolower) #Lowercase
  ) %>%
...
sum(grepl(" ", IMPC_parameter_clean$description)) #0 are missing _
...
IMPC_parameter_clean$name[!grepl("^[A-Z]", IMPC_parameter_clean$name)] #those that do not start with capitals start with numbers, %, special characters, or their format needed to be preserved this way
...
sum(grepl("^[A-Z]", IMPC_parameter_clean$description)) #0 begin with capital letter 
```
...
write.csv(IMPC_parameter_clean, file = "/scratch/grp/msc_appbio/DCDM/Group3/processed_data/IMPC_parameter_clean.csv",
          row.names = FALSE)
